...
const Page = () => {
  ...
  const handleSearchByCategory = (category) => {
    ...
    const tmp = products?.filter((product) => product?.category?.name.includes(category));
    console.log(tmp);
    setProductSearch(tmp);
  };
  ...
  useEffect(() => {
    getProductByPage(currentPage)
      .then((res) => {
        ...
        setProductSearch(res?.data?.product);
  ...
  return (
    <>
      ...
      <ModalCustomer open={open} setOpen={setOpen}  handleReload = {handleReload}/>
    </>
  );
};
