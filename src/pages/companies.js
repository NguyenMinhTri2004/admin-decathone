...
import { ModalSupplier } from "src/components/ModalSupplier";
import { applyPagination } from "src/utils/apply-pagination";
import { getProducts, getProductByPage } from "../api/product";
import { getUserByPage } from "../api/auth";
import { getSupplierByPage } from "src/api/supplier";
...
const Page = () => {
  ...
  const handleSearchByName = (e) => {
    ...
    const tmp = products?.filter((product) => product?.name?.includes(e?.target?.value));
    console.log(tmp);
  ...
  const handleSearchByCategory = (category) => {
    ...
    const tmp = products?.filter((product) => product?.roles?.includes(category));
    console.log(tmp);
    setProductSearch(tmp);
  };

  useEffect(() => {
    getSupplierByPage(currentPage)
      .then((res) => {
        console.log(res);
        setProducts(res?.data?.supplier);
        setProductSearch(res?.data?.supplier);
